...
public class CreateOrgWith_IndAndTypeTest {

	@Test
	public void createOrgWith_IndAndType() throws IOException, InterruptedException {
...
//		String orgName = wb.getSheet("Org_Data").getRow(7).getCell(2).toString();
//		String indName = wb.getSheet("Org_Data").getRow(7).getCell(3).toString();
//		String indType = wb.getSheet("Org_Data").getRow(7).getCell(4).toString();
		//Advance/updated way of fetching data
		ExcelFileUtilities ex= new ExcelFileUtilities();
		String orgName =ex.fetchDataFromExcelFile("Org_Data", 7, 2)+rnum;
		String indName =ex.fetchDataFromExcelFile("Org_Data", 7, 3);
		String indType =ex.fetchDataFromExcelFile("Org_Data", 7, 4);
		ex.closeExcel();
...
//		driver.findElement(By.linkText("Organizations")).click();
		...
		// Identify industry DD
		WebElement industryDD = driver.findElement(By.name("industry"));
//		Select s1 = new Select(industryDD);
//		s1.selectByValue(indName);
		wutility.selectIndustryByValue(industryDD, indName);

		// Identify Industry type
		WebElement indTypeDD = driver.findElement(By.name("accounttype"));
//		Select s2 = new Select(indTypeDD);
//		s2.selectByValue(indType);
		wutility.selectIndustryType(indTypeDD, indType);
		...
		// Identify and verify newly created Industry with type
		WebElement newInd = driver.findElement(By.xpath("//span[@id='dtlview_Industry']/font"));
		if (newInd.getText().contains(indName)) {
			System.out.println("Create org industry name test pass");
		} else {
			System.out.println("Create org industry name test fail");
		}

		// Identify and verify newly created Industry with type
		WebElement newType = driver.findElement(By.xpath("//span[@id='dtlview_Type']/font"));
		if (newType.getText().contains(indType)) {
			System.out.println("Create org industry Type test pass");
		} else {
			System.out.println("Create org industry name test fail");
		}
